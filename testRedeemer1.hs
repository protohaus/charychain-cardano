...
data Donation = Donation 
    { dDonor :: !PaymentPubKeyHash
    , dDonation :: !Integer } deriving Show 

{-data Initiation = Initiation 
    { dInitiator :: !PaymentPubKeyHash} deriving Show-}

 

data DonationAction = MkDonation Donation
    deriving Show 
...
firstValidator :: DonationDatum -> DonationAction -> ScriptContext -> Bool
firstValidator dd redeemer ctx = 
    traceIfFalse "wrong input value" correctInputValue && 
    case redeemer of 
        MkDonation d@Donation{..} -> 
            traceIfFalse "insufficient amount"  (sufficicentD dDonation) &&
            traceIfFalse "existing Donor"       (existingDonor dDonor)
  ...
  where
    ...
    sufficicentD :: Integer -> Bool 
    sufficicentD amount = amount > 2000000 

    existingDonor :: PaymentPubKeyHash -> Bool 
    --is part of Datum? 
    ...
    (ownOutput, outputDatum) = case getContinuingOutputs ctx of
        ...
        _   -> traceError "expected exactly one continuing output"(ownOutput, outputDatum) = case getContinuingOutputs ctx of
...
typedValidator :: Scripts.TypedValidator Donating
typedValidator = Scripts.mkTypedValidator @Donating
